...
export function openInputPopup({
  ...
  // Money UX (all platforms):
  // 1) value > 0 => caret at the end
  // 2) value == 0 => clear field immediately
  if (input && input.dataset.ffMoneyFocus !== "1") {
    input.dataset.ffMoneyFocus = "1";
    const applyFocusUX = () => {
      try {
    ...
    input.addEventListener("focus", applyFocusUX);
    input.addEventListener("click", applyFocusUX);
  }
