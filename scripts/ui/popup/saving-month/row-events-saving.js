...
export function wireSavingRowInteractions({
  ...
  const applyEditUX = () => {
    ...
    const run = () => {
      try {
        // UX rule: if value is 0 => empty; else caret at end
        const num = parseMoneyInput(amountInput.value);
        ...
        // ignore (e.g. unsupported selection range on some input types)
      }
    };
    run();
    setTimeout(() => { if (document.activeElement === amountInput) run(); }, 0);
    setTimeout(() => { if (document.activeElement === amountInput) run(); }, 60);
  };
  ...
  const applyMoneyFocusUX = () => {
    if (!amountInput) return;
    // Rule:
    // - value > 0 => caret at end
    // - value == 0 => clear field
    try {
      const n = parseMoneyInput(amountInput.value);
      if (n != null && Number.isFinite(n) && Math.abs(n) === 0) {
        amountInput.value = "";
      }
    } catch (_) {}

    // caret at end (iPhone-safe)
    try {
      const v = String(amountInput.value ?? "");
      amountInput.setSelectionRange(v.length, v.length);
    } catch (_) {}
    try {
      setTimeout(() => {
        try {
