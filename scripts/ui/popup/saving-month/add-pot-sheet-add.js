...
export function openAddSavingPotSheet({ year, onComplete } = {}) {
  ...
  if (yearsContainer) {
    const block = renderSavingYearRow(yearsContainer, y, 0, null);
    // UX: on "Spaarpot toevoegen" the initial month amount must start empty (no 0).
    try {
      const inp = block?.querySelector?.(".cat-budget-input");
      if (inp && String(inp.value ?? "").trim() === "0") inp.value = "";
    } catch (_) {}
  }
