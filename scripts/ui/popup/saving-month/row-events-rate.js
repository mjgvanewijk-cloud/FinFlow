...
function _applyRateFocusUX(input) {
  if (!input) return;
  ...
  try {
    ...
    if (raw) {
      ...
      if (Number.isFinite(num) && num === 0) input.value = "";
    }
  } catch (_) {}
  ...
  try {
    ...
    try { requestAnimationFrame(place); } catch (_) { place(); }
  } catch (_) {}
}
...
export function wireSavingRateInteractions({
  ...
  if (rateInput) {
    rateInput.addEventListener("click", (e) => { e.stopPropagation(); _applyRateFocusUX(rateInput); openEditor(); });
    rateInput.addEventListener("focus", (e) => { e.stopPropagation(); _applyRateFocusUX(rateInput); openEditor(); });
